...
$sql = "SELECT loans.id, books.title, CONCAT(readers.first_name, ' ', readers.last_name) AS full_name, DATE(loans.loan_date) AS loan_date, DATE(loans.due_date) AS due_date, loans.status FROM loans JOIN books ON loans.book_id = books.id JOIN readers ON loans.readers_id = readers.id WHERE books.title LIKE ? OR readers.first_name LIKE ? OR readers.last_name LIKE ?";
